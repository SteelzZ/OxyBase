 ...
 */
class Oxy_EventStore_Storage_Mongo implements Oxy_EventStore_Storage_Interface
{
    /**
     * Database adapter
     *
    ...
    {
        $this->_db = $db->$dbName;
        $this->_eventsCountToDoSnapShot = 1;
        $this->_version = array();
        $this->_dbName = $dbName;
    }
    ...
    public function getSnapShot(Oxy_Guid $eventProviderId)
    {
        ...
        try {
            $object = unserialize(base64_decode($result['snapshot']));
            $this->_version[(string)$eventProviderId] = $result['version'];
        } catch (Exception $ex) {
            return null;
        }
        return $object;
    ...
    public function getAllEvents(Oxy_Guid $eventProviderId)
    {
        ...
        $collection = $this->_db->events;
        $query = array(
            "aggregate_root_guid" => (string)$eventProviderId,
        );
    ...
    public function getEventCountSinceLastSnapShot(Oxy_Guid $eventProviderId)
    {
        ...
        $collection = $this->_db->events_count;
        $query = array(
            "guid" => (string)$eventProviderId
        );
    ...
    public function getEventsSinceLastSnapShot(Oxy_Guid $eventProviderId)
    {
        ...
        $whereToStart = 0;
        if(!is_null($cursor)) {
           $whereToStart = $cursor['at_event_id'];
        }
    ...
    private function shouldWeDoSnapShot($guid)
    {
        ...
        if (isset($cursor['count']) && ((int)$cursor['count'] === (int)$this->_eventsCountToDoSnapShot)) {
            return true;
        } elseif (isset($cursor['count']) && ((int)$cursor['count'] > (int)$this->_eventsCountToDoSnapShot)) {
            $this->updateCount(-$cursor['count'], $guid);
            return true;
        } else {
            return false;
        }
    ...
    private function saveChanges(Oxy_Domain_Event_Container_ContainerInterface $events, $guid)
    {
        ...
        $batchInsert = array();
        $result = $sequence->findOne();
        if(is_null($result)){
            $sequence->insert(array('id' => 1));
        }
        $result = $sequence->find()->sort(array('id' => -1));
        $nextId = $result->getNext();
        $seqId = $nextId['id'];
        ...
        foreach ($events->getIterator() as $key => $eventData) {
            ...
            $data = array(
                ...
                'aggregate_root_guid' => (string)$guid,
            ...
            //$batchInsert[] = $data;
            $collection->insert($data);
            //$collection->update(array(''), $data, array("upsert" => true));
            $sequence->insert(array('id' => $seqId));
        }
        //$collection->insert($batchInsert);
//        $lastId = $this->_db->lastInsertId();
    ...
    private function checkVersion(Oxy_EventStore_EventProvider_Interface $eventProvider)
    {
        ...
        $collection = $this->_db->events;
        $query = array(
            "aggregate_root_guid" => (string)$eventProvider->getGuid()
        );
        ...
        if ((int)$cursor['version'] === (int)$eventProvider->getVersion()) {
            return true;
        } else {
            return false;
        }
...
//            $this->_db->insert($this->_dbName . '.events_count', $data);
            ...
            $data = array('$set' => array('count' => ((int)$cursor['count'] + (int)$cnt)));

            $collection->update(array("guid" => (string)$guid), $data);
        }
    }
    ...
    {
        $collection = $this->_db->snapshots;
        $data = array('$set' => array('is_latest' => 0));
