...
class Oxy_Navigation_Page_Mvc extends Zend_Navigation_Page_Mvc
{
    ...
    public function getHref ()
    {
        ...
        if (null === parent::$_urlHelper) {
            parent::$_urlHelper = Zend_Controller_Action_HelperBroker::getStaticHelper('Url');
        }
        ...
        $url = parent::$_urlHelper->url($arrParams, $this->getRoute(), $this->getResetParams());
    ...
    public function setDomain ($strDomain)
    {
        if (null !== $strDomain && ! is_string($strDomain)) {
            throw new Oxy_Navigation_Exception(
                'Invalid argument: $strModule must be a string or null');
        }
